...
async def dashboard(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if not poll:
        return layout(TITLE, f"<h1>Daily Poll</h1><p>No draft exists for today.</p><form method=post action='/admin/polls/generate'><input type=hidden name=csrf value='{csrf}'><button>Generate draft</button></form>", current="/admin/polls")
    ...
    return layout(TITLE, f"<h1>Daily Poll — {poll.poll_date}</h1><p class=meta>Status: {poll.status} · Publishes 9:00 AM IST</p>{source}<form method=post action='/admin/polls/update'><input type=hidden name=csrf value='{csrf}'><input type=hidden name=poll_id value='{poll.id}'><label>Question<textarea name=question required>{html.escape(poll.question)}</textarea></label><label>Context<textarea name=context required>{html.escape(poll.context)}</textarea></label>{option_inputs}{controls}</form>", current="/admin/polls")


def _error_page(csrf: str, message: str) -> HTMLResponse:
    return layout(TITLE, f"<h1>Daily Poll</h1><p class=danger>Draft generation failed: {html.escape(message)}</p><form method=post action='/admin/polls/generate'><input type=hidden name=csrf value='{csrf}'><button>Try again</button></form>", current="/admin/polls")
