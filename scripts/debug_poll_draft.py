...
nothing to the database. Use this when generate_poll_now.py fails with
"Unsafe or invalid poll output" and you need to see what Claude actually
returned.
...
from app.services.polls import UNSAFE, validate_draft
...
async def main():
    ...
    async with AsyncSessionLocal() as session:
        ...
        print(f"{len(candidates)} candidate clusters in last 24h, {len(safe)} passed the UNSAFE headline filter")
        for cluster in safe:
            print(f"  [{cluster.id}] {cluster.headline}")
        if not safe:
            print("No safe corroborated stories available — nothing to send to Claude")
            return
        ...
        try:
            ...
            print(f"  UNSAFE match on question: {UNSAFE.search(question)}")
